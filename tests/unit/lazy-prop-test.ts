...
module('@lazyProp', function() {
    test(`lazyProps can be defined in ${DEBUG ? 'development' : 'production'}`, async function(assert) {
        try {
            class TestObject {
                @lazyProp
            ...
            new TestObject();
            assert.ok(true, `Defined lazyProp`);
        } catch (e) {
            assert.ok(false, `Unable to define lazyProp in ${DEBUG ? 'development' : 'production'}: ${e.message}`);
        }
    });
