...
module('DebugProxy', function() {
    test(`DebugProxy can be used`, async function(assert) {
        try {
            ...
            assert.ok(false, `Unable to use DebugProxy in ${DEBUG ? 'development' : 'production'}: ${e.message}`);
        }
    });
    ...
    test(`DebugProxy returns an instantiable class`, async function(assert) {
        try {
            ...
            assert.ok(false, `Unable to instantiate via DebugProxy in ${DEBUG ? 'development' : 'production'}: ${e.message}`);
        }
    });
    ...
    test(`DebugProxy passes instanceof checks`, async function(assert) {
        ...
        assert.ok(instance instanceof TestObject, `Passes instanceof check in ${DEBUG ? 'development' : 'production'}`);
    });
    ...
    if (DEBUG) {
        ...
        test(`DebugProxy does not shield private properties in production`, async function(assert) {
            ...
            const Test = DebugProxy(TestObject);
            let instance = new Test();
            assert.equal(instance.myProp, 'abc', 'We do not throw an error when accessing a private property in production');
            assert.equal(instance.myProp = '123', 'abc', 'We do not throw an error when setting a private property in production');
        ...
        test(`DebugProxy does not shield private methods in production`, async function(assert) {
            ...
            const Test = DebugProxy(TestObject);
            let instance = new Test();
            assert.equal(instance.myProp('123'), '123', 'We can invoke private methods in production');
        });
    }
});